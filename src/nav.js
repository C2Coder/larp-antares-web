...
export function createNav(activePage) {
  const links = [
    ...
    { id: 'o-nas',        href: '/o-nas',          label: 'Kontakt' },
  ];

  const desktopLinks = links.map(({ id, href, label }) => {
    const active = id === activePage ? 'border-b-2 border-primary-dark text-primary-dark' : '';
    const current = id === activePage ? 'aria-current="page"' : '';
    return `<a href="${href}" ${current} class="text-header-text hover:text-primary-dark text-sm xl:text-base fc font-semibold transition-colors ${active}">${label}</a>`;
  }).join('\n          ');
  ...
  const mobileLinks = links.map(({ id, href, label }) => {
    ...
    return `<a href="${href}" class="block py-2 ${active} text-header-text hover:text-primary-dark fc font-semibold transition-colors" data-mobile-link>${label}</a>`;
  }).join('\n          ');
  ...
  <header class="fixed top-0 left-0 right-0 z-50 bg-header/95 backdrop-blur shadow-sm overflow-visible">
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex items-center gap-1">
          <a href="https://instagram.com/larpantares/" target="_blank" rel="noopener noreferrer" class="p-2 text-header-text hover:text-primary transition-colors">
          ...
          <a href="https://www.facebook.com/AntaresLARP" target="_blank" rel="noopener noreferrer" class="p-2 text-header-text hover:text-primary transition-colors">
    ...
    <div id="mobile-menu" class="hidden md:hidden bg-header/98 backdrop-blur border-t border-header-text/20" aria-label="Mobilní navigace">
